...
CLIENT_SECRETS_FILE = "/secrets/admob_client_secrets.json"
...
TOKEN_FILE = '/secrets/admob_token.pickle'
...
def _parse_raw_query_params(data):
  ...
  match = re.search(r"GET\s+\/\?([^&\s]+(?:&[^&\s]+)*)", decoded)
  if not match:
    print("Error: Could not parse authorization response. Raw data:", decoded)
    sys.exit(1)
